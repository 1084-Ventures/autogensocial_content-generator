...
#   timestamp: 2025-06-14T19:30:15+00:00
...
class AzureOpenAIGenerateContentTemplate(BaseModel):
    name: str = Field(..., description='Name of the template')
    content: str = Field(..., description='Content or structure of the template')


class AzureOpenAIGenerateContentRequest(BaseModel):
    template: AzureOpenAIGenerateContentTemplate = Field(
        ..., description='Template object for content generation'
    )
    variable_values: Optional[dict[str, str]] = Field(
        None, alias='variableValues', description='Variable values for the template'
    )


class AzureOpenAIGenerateContentResponse(BaseModel):
    text: Optional[str] = Field(None, description='The generated text content')
    image_description: Optional[str] = Field(
        None,
        alias='imageDescription',
        description='Description of the type of image to generate (if any)',
    )
    commentary: Optional[str] = Field(None, description='Additional commentary for the post')
    hashtags: Optional[list[str]] = Field(None, description='List of hashtags for the post')


class OrchestratorRequest(BaseModel):
    template_id: str = Field(..., alias='templateId')
    brand_id: str = Field(..., alias='brandId')
    variable_values: list = Field(default_factory=list, alias='variableValues')


class OrchestratorResponse(BaseModel):
    status: str = Field(..., description='The status of the operation')
    result: Optional[dict[str, Any]] = Field(None, description='The result of the operation')
    error: Optional[str] = Field(None, description='Error message if the operation failed')


class PostingRequest(BaseModel):
    brand_id: str = Field(..., description='The brand ID')
...
class PostingResponse(BaseModel):
    status: str = Field(..., description='The status of the post operation')
    post_url: Optional[str] = Field(None, description='The URL of the posted content')
    error: Optional[str] = Field(None, description='Error message if the operation failed')
...
class Background(BaseModel):
    type: str = Field(..., description='The type of background (e.g., color, image)')
    value: str = Field(
        ..., description='The value for the background (e.g., color code or image URL)'
    )
    filters: Optional[list[str]] = Field(
        None, description='Optional filters to apply to the background'
    )
...
class Font2(BaseModel):
    ...
    size: str = Field(..., description='Font size (e.g., "32px").')
    weight: Optional[Weight] = Field(None, description='Font weight for the overlay text.')
    style: Optional[Style3] = Field(None, description='Font style for the overlay text.')
...
class TextOverlay(BaseModel):
    text: str = Field(..., description='The text to overlay on the image')
    font: Font2
    color: Color
    position: Position3 = Field(..., description='The position of the text overlay')


class MediaSearchRequest(BaseModel):
    text: str = Field(..., description='The text to search for related media')
    source: Optional[str] = Field(
        'internal', description="The source to search (e.g., 'internal', 'online')"
    )


class MediaSearchResponse(BaseModel):
    media: dict[str, Any] = Field(..., description='The best match media result')
    source: str = Field(..., description='The source of the media')
